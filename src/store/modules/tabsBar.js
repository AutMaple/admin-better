...
const actions = {
  ...
  async delOthersRoutes({ dispatch, state }, route) {
    await dispatch("delOthersRoutes", route);
    return {
      visitedRoutes: [...state.visitedRoutes],
    };
  },
...
const mutations = {
  ...
  delLeftVisitedRoute(state, route) {
    ...
    state.visitedRoutes = state.visitedRoutes.filter((item) => {
      ...
      return item.media.affix || index <= state.visitedRoutes.indexOf(item);
    });
  },
...
const getters = { visitedRoutes: (state) => state.visitedRoutes };
